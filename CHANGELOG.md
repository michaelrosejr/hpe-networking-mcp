...
- **`.env`-file / environment-variable credentials** (contributed by @michaelrosejr, #619). Credentials may now come from a `.env` file / environment variables in addition to Docker secret files: `_read_secret()` reads the secret file at `SECRETS_DIR/<name>` first (production priority) and falls back to the `NAME.upper()` env var. Adds `.env.example` and a `docker-compose.secrets.yml` overlay for the file-secret path. The default `docker-compose.yml` stays **loopback-bound (`127.0.0.1`)** — expose to `0.0.0.0` only behind an authenticating proxy/firewall.
